...
+~critical(X,Y): not allVictim(_)
    <- .print("THE VICTIM AT ", X, ",", Y, " IS NOT CRITICAL");
		noCritical(0);
	    nextVictim(_).
		
+~critical(X,Y): allVictim(_)
    <- 	noCritical(1);
		!pick(v);
	    !returnH(_).
		
+atH(_): not noVictim(_)
	<- .print("FIND NEXT VICTIM"); 
	nextVictim(_).
...
+white(_): true
	<- .print("THIS IS NOT A VICTIM");
		white(_);
		nextVictim(_).

+!pick(v)
	<- .print("PICK UP VICTIM");
	   pickUpVictim(_).
